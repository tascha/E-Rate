...
```{r}
# Loop through each year's outlet dataframes and add in the PLS_YEAR column
...
for (i in 1:length(outlets)) {
  all_outlets_1[[i]] <- outlets[[i]] %>%
    # Add a column called PLS_YEAR with the year
...
```{r}
# remove the list now that it's a dataframe
rm(all_outlets_1)
```
...
```{r}
# remove the lists and temp dfs
rm(all_aes_with_FSCS_SEQ, all_aes, so_not_bookmobile, bookmobiles, admin_not_sep)
```
...
```{r}
# How many rows exist in the aes_and_outlets_20142020 df where the FSCS_SEQ is NA?
aes_and_outlets_20142020 %>% filter(is.na(FSCS_SEQ)) %>% nrow()
```
