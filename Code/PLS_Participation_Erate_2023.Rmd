...
We can produce a table that provides a summary of the data as you described:

    1)  xxx libraries serve >100,000 pop. And xxx of these apply for the E-rate.
    2)  xxx libraries serve 50,000 – 99,999 pop. And xxx of these apply for the E-rate.
    3)  xxx libraries serve 25,000 – 49,999  pop. And xxx of these apply for the E-rate.
    4)  xxx libraries serve 5,000 – 24,999 pop. And xxx of these apply for the E-rate.
    5)  xxx libraries serve <5,000 pop. And xxx of these apply for the E-rate.

```{r}
# Add in requested population categories
erate_apps_2023 <- erate_apps_2023 |> 
  mutate(POP_CATEGORY = case_when(
    POPU_LSA == -3 ~ "No Population closed or temporarily closed administrative entity",
    POPU_LSA == -9 ~ "Population data suppressed for analytic purposes",
    is.na(POPU_LSA) ~ "Population unknown",
      POPU_LSA < 5000 ~ "Under5k",
      POPU_LSA >= 5000 & POPU_LSA < 25000  ~ "5to25K",
      POPU_LSA >= 25000 & POPU_LSA < 50000  ~ "25to50K",
      POPU_LSA >= 50000 & POPU_LSA < 100000  ~ "50to100K",
      POPU_LSA >= 100000 ~ "100Kover"))
```

```{r}
# check for dupes
erate_apps_2023 %>% distinct(ros_entity_number, ros_entity_name, FSCSKEY, FSCS_SEQ) |> group_by(FSCSKEY, FSCS_SEQ) |> add_tally() |> filter(n>1 & n < 5) |> arrange(FSCSKEY, FSCS_SEQ)
```


```{r}
# write out a csv of select columns
write.csv(erate_apps_2023, "~/Documents/GitHub/E-Rate/Data/2023_Erate_Applications_Libs_and_LibSystems_with_FSCS.csv")
```

```{r}
# make a dataset of IMLS outlets to use for analysis
imls_out_analyze <- imls_out_2020 |> 
  filter(STATSTRU != 3) |> 
  select(FSCSKEY, FSCS_SEQ, LIBNAME, STABR, ADDRESS) |> 
  left_join(imls_ae_2020 |> mutate(FSCS_IMLS_Match = 'Yes') |> select(FSCSKEY, POPU_LSA, FSCS_IMLS_Match), by = "FSCSKEY") |> 
   mutate(POP_CATEGORY = case_when(
    POPU_LSA == -3 ~ "No Population closed or temporarily closed administrative entity",
    POPU_LSA == -9 ~ "Population data suppressed for analytic purposes",
    is.na(POPU_LSA) ~ "Population unknown",
      ...
      POPU_LSA >= 50000 & POPU_LSA < 100000  ~ "50to100K",
      POPU_LSA >= 100000 ~ "100Kover")) |> 
  left_join(erate_apps_2023 |> filter(!is.na(FSCSKEY) & !is.na(FSCS_SEQ)) |> distinct(FSCSKEY, FSCS_SEQ, ros_entity_number, ros_entity_name, .keep_all = TRUE) |> select(FSCSKEY, FSCS_SEQ, ros_entity_number, ros_entity_name, organization_entity_type_name),
            by = c("FSCSKEY", "FSCS_SEQ"))
```

```{r}
# write out a csv of select columns
write.csv(imls_out_analyze, "~/Documents/GitHub/E-Rate/Data/2020_PLS_with_USAC_Identifiers_from_2023_Erate_Apps.csv")
```
...
```{r}
# how are the dupes spread across pops?
imls_out_analyze %>% 
  group_by(FSCSKEY, FSCS_SEQ) |> 
  add_tally() |> 
  filter(n>1 & n < 10) |> 
  ungroup() |> 
  summarise(number = n_distinct(FSCSKEY, FSCS_SEQ))
```
...
```{r}
# there are legit reasons why some IMLS entities match multiple USAC entities
# we don't need to know those for this analysis though, so let's just keep the first occurrence
# when there is a duplicate FSCSKEY and FSCS_SEQ combo
imls_out_analyze <- imls_out_analyze |> 
  group_by(FSCSKEY, FSCS_SEQ) %>%
  slice_head(n=1) |> 
  ungroup()
```
...
```{r}
imls_out_analyze |> filter(FSCS_IMLS_Match == 'Yes') |> count(POP_CATEGORY)
```
