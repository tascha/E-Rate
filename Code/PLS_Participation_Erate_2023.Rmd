  ...
  left_join(imls_ae_2020 |> select(FSCSKEY, POPU_LSA), by = "FSCSKEY") |> 
   mutate(POP_CATEGORY = case_when(
    ...
    is.na(POPU_LSA) ~ "Population unknown",
      ...
      POPU_LSA >= 50000 & POPU_LSA < 100000  ~ "50to100K",
      POPU_LSA >= 100000 ~ "100Kover")) |> 
  left_join(erate_apps_2023 |> filter(!is.na(FSCSKEY) & !is.na(FSCS_SEQ)) |> distinct(FSCSKEY, FSCS_SEQ, ros_entity_number, ros_entity_name),
...
```{r}
imls_out_analyze |> count(POP_CATEGORY)
```
