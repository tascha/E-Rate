---
title: "Cat 1 and 2 Participation Rates by State"
...
# Only want to include hand matches that exist in the 2021 PLS
...
```{r}
# Count the nunmber of imls entities by state only BR and CE entities
imls_out_2021 |> 
  filter(C_OUT_TY %in% c('BR', 'CE')) |> 
  group_by(STABR) |> 
  summarise(Entities_2021 = n())
```

```{r}
# Count entities in erate dataset by State and funding year
erate |> 
  filter(C_OUT_TY %in% c('BR', 'CE'),
         !is.na(FSCSKEY)) |> 
  group_by(ros_physical_state, chosen_category_of_service, funding_year) |> 
  summarise(entities_committed = n_distinct(ros_entity_number)) |> 
  pivot_wider(names_from = chosen_category_of_service, values_from = entities_committed) |> 
  left_join(imls_out_2021 |> 
              filter(C_OUT_TY %in% c('BR', 'CE')) |>
              group_by(STABR) |>
              summarise(Entities_2021 = n()),
            join_by('ros_physical_state' == 'STABR'))
```
...
erate |> 
  ...
  left_join(imls_out_2021 |> 
              ...
              summarise(Entities_2021 = n()),
            join_by('ros_physical_state' == 'STABR')) |> 
  mutate(across(Category1:Category2, ~ round(.x/Entities_2021, 3))) |> 
  select(-Entities_2021) |> 
  write.csv("~/Documents/GitHub/E-Rate/Data/Commitment_Percentages_by_State.csv")

```

```{r}
# What can we tell about MO
erate |> 
  filter(ros_physical_state == "MO") |> 
  group_by(funding_year, chosen_category_of_service) |> 
