...
write.csv(imls_out_2020, "~/Documents/GitHub/E-Rate/Data/2020_Open_IMLS_Outlets_with_2022_Erate_Cat1_Commitment_Binary_Column.csv")
```
...
```{r}
ia_data_only |> 
  group_by(how_many_rows_per_lib) |> 
  summarise(n_distinct(ros_entity_number))
```
...
```{r}
# How many libs have download speed of at least 1000
ia_data_only |> 
  select(ros_entity_number, FSCSKEY, FSCS_SEQ, pop_category, download_speed_mbps, how_many_rows_per_lib) |> 
  group_by(ros_entity_number) |> 
  arrange(ros_entity_number, desc(download_speed_mbps)) |> 
  slice_head(n=1) |> 
  ungroup() |> 
  filter(download_speed_mbps > 999) |> 
  summarise(n_distinct(ros_entity_number))
```


```{r}
# How many libraries meet the FCC target?
ia_data_only |> 
...
```{r}
ia_data_only |> 
  select(ros_entity_number, FSCSKEY, FSCS_SEQ, form_471_function_name, speed_benchmark, how_many_rows_per_lib) |> 
  group_by(ros_entity_number) |> 
  arrange(ros_entity_number, speed_benchmark) |> 
  slice_head(n=1) |> 
  ungroup() |> 
  group_by(form_471_function_name, speed_benchmark) |> 
  summarise(n_distinct(ros_entity_number))
```
