...
noterate <- dplyr::setdiff(imls_out_2021 |> distinct(FSCSKEY, FSCS_SEQ), cat1_2022 |> filter(ros_entity_type != "non-instructional facility (nif)") |> distinct(FSCSKEY, FSCS_SEQ)) |> nrow()
```

```{r}
# Output the outlets that do not match to an entity in the Cat1 dataset
dplyr::setdiff(imls_out_2021 |> distinct(FSCSKEY, FSCS_SEQ), cat1_2022 |> filter(ros_entity_type != "non-instructional facility (nif)") |> distinct(FSCSKEY, FSCS_SEQ))
```

```{r}
# How many PLS outlets are also in our Cat1 dataset?
nrow(imls_out_2021) - noterate
```
...
write.csv(imls_out_2021, "~/Documents/GitHub/E-Rate/Data/2020_IMLS_CEandBR_Outlets_with_2022_Erate_Cat1_Commitment_Binary_Column.csv")
```
...
(nrow(imls_out_2021) - noterate)/nrow(imls_out_2021)
```
