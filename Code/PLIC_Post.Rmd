...
```{r}
# We only want outlets that are classified as central or branch (not bookmobiles or kiosks)
...
cat1_2022 <- cat1_2022 |> 
  ...
  left_join(imls_out_2020 |> select(FSCSKEY, FSCS_SEQ, LOCALE, LOCALE_DESCR, LOCALE_TOP_LEVEL_DESCR), by = c("FSCSKEY", "FSCS_SEQ"))
```
...
# counts the FSCS that do not have a match in the cat1 dataset
# filter out NIFs first from Cat1 dataset
noterate <- dplyr::setdiff(imls_out_2020 |> distinct(FSCSKEY, FSCS_SEQ), cat1_2022 |> filter(ros_entity_type != "non-instructional facility (nif)") |> distinct(FSCSKEY, FSCS_SEQ)) |> nrow()
```

```{r}
# Output the outlets that do not match to an entity in the Cat1 dataset
dplyr::setdiff(imls_out_2020 |> distinct(FSCSKEY, FSCS_SEQ), cat1_2022 |> filter(ros_entity_type != "non-instructional facility (nif)") |> distinct(FSCSKEY, FSCS_SEQ))
```
...
```{r}
# Replace NA with false in one column
imls_out_2020 <- imls_out_2020 |> 
  mutate(Cat1_Commitment_2022 = ifelse(is.na(Cat1_Commitment_2022), FALSE, Cat1_Commitment_2022))
```

```{r}
# Save the imls_out_2020 file to Github for reference
write.csv(imls_out_2020, "~/Documents/GitHub/E-Rate/Data/2020_IMLS_CEandBR_Outlets_with_2022_Erate_Cat1_Commitment_Binary_Column.csv")
```
...
write.csv(ia_purposes, "~/Documents/GitHub/E-Rate/Data/2022_Internet_Access_with_Purpose_IMLS_Entities_Single_Row.csv")
```
...
```{r}
# How many libs meet the fcc benchmark
ia_purposes |> 
...
```{r}
# Find the denominator of how many libs are in each category
imls_out_2020 |> 
...
```{r}
ia_purposes |> 
  select(ros_entity_number, FSCSKEY, FSCS_SEQ, LOCALE_TOP_LEVEL_DESCR, fcc_speed_benchmark, how_many_rows_per_lib) |> 
  group_by(ros_entity_number) |> 
  arrange(ros_entity_number, fcc_speed_benchmark) |> 
  slice_head(n=1) |> 
  ungroup() |> 
...
# Find the denominator of how many libs are in each category
imls_out_2020 |> 
  group_by(LOCALE_TOP_LEVEL_DESCR) |> 
  summarise(n_distinct(FSCSKEY, FSCS_SEQ))
```

```{r}
ia_purposes |> 
  select(ros_entity_number, FSCSKEY, FSCS_SEQ, form_471_function_name, fcc_speed_benchmark, how_many_rows_per_lib) |> 
  group_by(ros_entity_number) |> 
  arrange(ros_entity_number, fcc_speed_benchmark) |> 
  slice_head(n=1) |> 
  ungroup() |> 
