...
```{r}
# For this analysis we are using only imls_outlets from 2020 that were not closed
# For comparison-sake remove closed libs from imls_out_2020
imls_out_2020_comp <- imls_out_2020 |> 
  filter(STATSTRU != 3)
```

```{r}
# get the list of libs that were closed in the outlets_2020 dataset
closed <- imls_out_2020 |> 
  filter(STATSTRU == 3)
```


```{r}
# How many unique entities in erate 2022 applied for Cat1?
erate_imls |> 
...
erate_2022 <- erate_imls %>% 
  filter(chosen_category_of_service == "Category1",
         ...
         FSCS_SEQ != 999,
         !is.na(FSCS_SEQ)) %>% 
  # Remove rows that are in the closed dataset
  # https://stackoverflow.com/a/41371660
  anti_join(closed, by = c("FSCSKEY", "FSCS_SEQ")) |> 
  left_join((purpose %>% 
...
write.csv(erate_2022, "~/Documents/GitHub/E-Rate/Data/Internet_Speed_Applications_2022.csv")
```

```{r}
# How many imls entities are in our dataset?
erate_2022 |> filter(chosen_category_of_service == "Category1") |> 
  summarise(n_distinct(FSCSKEY, FSCS_SEQ), n_distinct(ros_entity_number))
```

```{r}
erate_2022 |> 
  group_by(FSCSKEY, FSCS_SEQ) |> 
  mutate(count_ents = n_distinct(ros_entity_number)) |> 
  filter(count_ents > 1) |> 
  arrange(FSCSKEY, FSCS_SEQ) |> 
  write.csv("~/Documents/GitHub/E-Rate/Data/PLS_entities_matching_multiple_RENS.csv")
```
...
```{r}
# output csv of USAC libraries not matched with IMLS
erate_imls |> 
  filter(is.na(FSCSKEY)) |> 
  distinct(ros_entity_number, ros_entity_name, ros_entity_type, ros_physical_address, ros_physical_city, ros_physical_state, ros_physical_zipcode) |> 
  write.csv("~/Documents/GitHub/E-Rate/Data/USAC_Libraries_Not_Matched_to_IMLS.csv")
```
...
erate_2022 <- erate_2022 %>% 
  left_join(full_imls %>%
              ...
              select(FSCSKEY, FSCS_SEQ, LOCALE_ADD, LOCALE, LOCALE_ADD_DESCR, LOCALE_TOP_LEVEL_DESCR),
...
```{r}
# Coun the number of distinct entities that met the bandwidth target by the population
erate_2022 |> 
